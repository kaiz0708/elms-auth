...
@Slf4j
@Data
public class LandingIsJwt implements Serializable {
    ...
    public static LandingIsJwt decode(String input){
        LandingIsJwt result = null;
        try {
            String[] items = ZipUtils.unzipString(input).split(DELIM,11);
            if(items.length >= 10){
                result = new LandingIsJwt();
